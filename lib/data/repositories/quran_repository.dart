...
/// NOTE: This provider MUST be overridden in main.dart
...
class QuranRepository {
  ...
  QuranRepository(this._db, this._prefs);
  ...
  Future<void> _downloadFullQuranAudio(
    ...
    int totalSurahsProcessed = 0;

    // OPTIMIZATION 4: Parallelize Surah processing (BLAZING FAST)
    // INCREASED BATCH: 5 -> 10
    await _processInBatches<int>(
      surahIds,
      10,
      (surahNum) async {
        AppLogger.d(
            "Repo: Starting Audio Download for Surah $surahNum (Reciter: $reciterId)...");
        await downloadSurahAudio(surahNum, reciterId, (surahPct) {
          // Inner progress ignored for cleaner logs, focusing on completion
        });
        AppLogger.d("Repo: Completed Audio for Surah $surahNum");
        totalSurahsProcessed++;

        final overallProgress = totalSurahsProcessed / 114;
        if ((overallProgress * 100).toInt() % 5 == 0) {
          onProgress("Downloading Audio: ${(overallProgress * 100).toInt()}%");
        }
      },
    );
  }
  ...
  Future<List<Edition>> getAllTranslationEditions() async {
    ...
    if (c.isNotEmpty) return c.map(Edition.fromData).toList();
  ...
  Future<List<Reciter>> getAllReciters() async {
    ...
    if (c.isNotEmpty) return c.map(Reciter.fromData).toList();
  ...
  Future<List<Surah>> getAllSurahs() async =>
      (await _db.getAllSurahs()).map(Surah.fromData).toList();
  ...
  Future<List<AyahWithTranslations>> getAyahsForSurah(
    ...
    return _db.getAyahsForSurah(
        surahNumber, editionIdentifiers, wordByWordEdition, reciterIdentifier,
  ...
  Future<void> deleteAllLocalData() async {
    ...
    try {
      ...
      AppLogger.e("Repo: Error closing DB", e);
    }
    try {
      final dir = await getApplicationDocumentsDirectory();
      ...
      AppLogger.e("Repo: Error deleting physical files", e);
    }
  }
  ...
  Future<List<QuranSearchResult>> searchAyahs(String query) async {
    ...
    try {
      // We pass the query to the DB.
      // The DB handles the JOINs and the LIKE %...% logic.
      return await _db.searchContent(query.trim());
    } catch (e) {
      AppLogger.e("Repo: Search failed for '$query'", e);
      return [];
    }
  }
...
final quranRepositoryProvider = Provider<QuranRepository>((ref) {
  ...
  return QuranRepository(db, prefs);
});
