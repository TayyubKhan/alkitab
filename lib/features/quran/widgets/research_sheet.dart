...
class ResearchSheet extends ConsumerStatefulWidget {
  ...
  final Surah surah;
  const ResearchSheet({super.key, required this.ayah, required this.surah});
              ...
              children: [
                ...
                Expanded(
                  child: ListView.builder(
                    ...
                    // Increase bottom padding to account for input area + keyboard if needed,
                    // though usually the input moves up and covers the list bottom, so we just need
                    // enough padding for the static input area height.
                    padding: const EdgeInsets.fromLTRB(16, 16, 16, 180),
                    itemCount: messages.length,
                    itemBuilder: (context, index) {
                      return MessageItemRenderer(message: messages[index]);
                    },
                  ),
                ),
              ],
        ...
        children: [
          ...
          Row(
            ...
            children: [
              ...
              Text("AI Research",
                  style:
                      theme.textTheme.labelLarge?.copyWith(letterSpacing: 1.0)),
            ],
          ),
...
class ResearchInputArea extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final theme = Theme.of(context);
    return ClipRRect(
      // Removed BackdropFilter for performance or kept it minimal
      // If lag persists, we can remove this ClipRRect/BackdropFilter entirely.
...
class MessageItemRenderer extends StatelessWidget {
  ...
  const MessageItemRenderer({super.key, required this.message});
...
// -----------------------------------------------------------------------------
// GRAMMAR CARD (Updated with PageView)
// -----------------------------------------------------------------------------
...
class _GrammarCardState extends State<GrammarCard> {
  // PageController helps us control swiping
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(
        children: [
          // 1. Horizontal Scrollable Area
          SizedBox(
            height: 320, // Fixed height specifically for the card content
        ...
        Expanded(
          child: Column(
            children: [
              ...
              Divider(height: 1, color: borderColor),
              // Use Expanded + SingleChildScrollView to fix Overflow
              Expanded(
                child: Center(
...
// -----------------------------------------------------------------------------
// HISTORY & INSIGHT CARDS (Updated with Safety Checks)
// -----------------------------------------------------------------------------
